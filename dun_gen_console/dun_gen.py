...
class Dungeon:
    ...
    def __init__(self,cell_count:tuple[int,int]=(4,3),map_size:tuple[int,int]=(96,39),mode:int=0,density:float=0.6, merge_chance:float=0.05,search_range:int=1,debug=False,**kwargs):
        ...
        if self.debug:
            print("Debugging on")
            self.sequence=[]
            self.debug_map=np.full((self.map_height,self.map_width),self._cm['B'],dtype='uint8')
            self.debug_map[1:-1,1:-1]=self._cm['W']
    ...
    def setup(self):
        ...
        for pos,cell in self.cells.items():
            ...
            if self.debug:
                ...
                self.debug_map[oy,ox]=self._cm['R' if cell.cell_type=='Room' else 'C']
                self.debug_map[oy+dy-1,ox+dx-1]=self._cm['R' if cell.cell_type=='Room' else 'C']
                t=Image.fromarray(self.debug_map)
                t.putpalette(self._debug_palette)
                self.sequence.append(t.copy())
        ...
        for (x,y),cell in self.cells.items():
            ...
            for path in cell.connections:
                ...
                if y==by:#horizontal
                    ...
                    if self.debug:
                        self.debug_map[l_ep,ox+dx-1]=self._cm['P']
                        self.debug_map[r_ep,obx]=self._cm['P']
                        self.debug_map[l_ep,mp]=self._cm['P']
                        self.debug_map[r_ep,mp]=self._cm['P']
                        t=Image.fromarray(self.debug_map)
                        t.putpalette(self._debug_palette)
                        self.sequence.append(t.copy())
                    ...
                    if self.debug:
                        self.debug_map[oy+dy-1,t_ep]=self._cm['P']
                        self.debug_map[oby,b_ep]=self._cm['P']
                        self.debug_map[mp,t_ep]=self._cm['P']
                        self.debug_map[mp,b_ep]=self._cm['P']
                        t=Image.fromarray(self.debug_map)
                        t.putpalette(self._debug_palette)
                        self.sequence.append(t.copy())
